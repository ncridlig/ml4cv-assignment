...
- Optimized for 16GB VRAM (Hiera-Base + efficient decoder)
"""
...
# -----------------------------
# Hiera pretrained models (hiera_base_224, hiera_small_224, etc.)
...
print("="*60)
print("HIERA SEMANTIC SEGMENTATION TRAINING")
print("="*60)
print(f"Device: {DEVICE}")
print(f"Batch size: {BATCH_SIZE}")
print(f"Learning rate: {LR}")
print(f"Epochs: {EPOCHS}")
print(f"Image size: {IMAGE_SIZE[0]}×{IMAGE_SIZE[1]} (Hiera pretrained resolution)")
...
class HieraSegmentation(nn.Module):
    ...
    def __init__(self, backbone_name='hiera_base_224', num_classes=13, pretrained=True):
        super().__init__()
        ...
        if backbone_name == 'hiera_small_224':
            self.backbone = hiera.hiera_small_224(
                pretrained=pretrained,
                checkpoint="mae_in1k_ft_in1k" if pretrained else None
            )
            # Hiera-Small: embed_dim=96, stages produce [96, 192, 384, 768]
            stage_channels = [96, 192, 384, 768]
        elif backbone_name == 'hiera_base_224':
            self.backbone = hiera.hiera_base_224(
                pretrained=pretrained,
                checkpoint="mae_in1k_ft_in1k" if pretrained else None
            )
            # Hiera-Base: embed_dim=96, stages produce [96, 192, 384, 768]
            stage_channels = [96, 192, 384, 768]
            ...
            # Hiera-Tiny: embed_dim=96, stages produce [96, 192, 384, 768]
            stage_channels = [96, 192, 384, 768]
        else:
...
# MODEL
# -----------------------------
# Use Hiera-Base for good balance of speed and accuracy
# Hiera-Small also works well if you need more VRAM headroom
model = HieraSegmentation(
    backbone_name='hiera_base_224',  # Change to 'hiera_small_224' for less VRAM
    num_classes=NUM_CLASSES,
    pretrained=True
)
